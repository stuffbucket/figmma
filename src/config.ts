...
export function getToken(): string | undefined {
  return process.env.FIGMA_API_TOKEN ?? process.env.FIGMA_TOKEN ?? readFile().figmaApiToken;
}

/** Get the Figma team ID. process.env wins, then config file. */
export function getTeamId(): string | undefined {
  return process.env.FIGMA_TEAM_ID ?? readFile().figmaTeamId;
}
...
export function getAll(): { token: string | undefined; teamId: string | undefined } {
  return { token: getToken(), teamId: getTeamId() };
}
